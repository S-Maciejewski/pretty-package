...
const getTableObject = async (
  packageJsonContent: string,
  options: Options
): Promise<TableObject | boolean> => {
  try {
    ...
    if (options.includeDevDependencies) {
      if (packageJson.devDependencies) {
        ...
        table.type = table.name.map((name) =>
          devDependencies.includes(name) ? "dev" : "prod"
        );
      } else {
...
const getMarkdownTable = (tableObject: TableObject): string => {
  ...
  const separator = ` | ${Object.keys(tableObject)
    .map(() => "---")
    .join(" | ")} | `;
